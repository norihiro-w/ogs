    ...
    BaseLib::ConfigTree const& config)
{
    ...
    config.checkConfigParameter("type", "THERMO_HYDRO_MECHANICS_WITH_LIE");
    DBUG("Create ThermoHydroMechanicsProcess with LIE.");
    auto const staggered_scheme =
        //! \ogs_file_param{prj__processes__process__THERMO_HYDRO_MECHANICS_WITH_LIE__coupling_scheme}
        config.getConfigParameterOptional<std::string>("coupling_scheme");
    const bool use_monolithic_scheme =
        !(staggered_scheme && (*staggered_scheme == "staggered"));

    // Process variables
    //! \ogs_file_param{prj__processes__process__THERMO_HYDRO_MECHANICS_WITH_LIE__process_variables}
    auto const pv_conf = config.getConfigSubtree("process_variables");
    auto range =
        //! \ogs_file_param{prj__processes__process__THERMO_HYDRO_MECHANICS_WITH_LIE__process_variables__process_variable}
        pv_conf.getConfigParameterList<std::string>("process_variable");
    std::vector<std::reference_wrapper<ProcessVariable>> p_T_u_process_variables;
    ...
    {
        if (pv_name != "pressure" && pv_name != "temperature" && pv_name != "displacement" &&
        ...
        {
            p_T_u_process_variables.emplace_back(
                const_cast<ProcessVariable&>(*variable));
        }
    }
    ...
    {
        process_variables.push_back(std::move(p_T_u_process_variables));
    }
    ...
    auto& intrinsic_permeability = ParameterLib::findParameter<double>(
        config,
        //! \ogs_file_param_special{prj__processes__process__THERMO_HYDRO_MECHANICS_WITH_LIE__intrinsic_permeability}
    ...
    auto& specific_storage = ParameterLib::findParameter<double>(
        config,
        //! \ogs_file_param_special{prj__processes__process__THERMO_HYDRO_MECHANICS_WITH_LIE__specific_storage}
    ...
    auto& biot_coefficient = ParameterLib::findParameter<double>(
        config,
        //! \ogs_file_param_special{prj__processes__process__THERMO_HYDRO_MECHANICS_WITH_LIE__biot_coefficient}
    ...
    auto& porosity = ParameterLib::findParameter<double>(
        config,
        //! \ogs_file_param_special{prj__processes__process__THERMO_HYDRO_MECHANICS_WITH_LIE__porosity}
    ...
    auto& solid_density = ParameterLib::findParameter<double>(
        config,
        //! \ogs_file_param_special{prj__processes__process__THERMO_HYDRO_MECHANICS_WITH_LIE__solid_density}
    ...
    Eigen::Matrix<double, GlobalDim, 1> specific_body_force;
    {
        std::vector<double> const b =
            //! \ogs_file_param{prj__processes__process__THERMO_HYDRO_MECHANICS_WITH_LIE__specific_body_force}
    ...
    auto const opt_fracture_model_config =
        //! \ogs_file_param{prj__processes__process__THERMO_HYDRO_MECHANICS_WITH_LIE__fracture_model}
    ...
    if (opt_fracture_model_config)
    {
        ...
        auto const frac_type =
            //! \ogs_file_param{prj__processes__process__THERMO_HYDRO_MECHANICS_WITH_LIE__fracture_model__type}
    ...
    for (auto fracture_properties_config :
        //! \ogs_file_param{prj__processes__process__THERMO_HYDRO_MECHANICS_WITH_LIE__fracture_properties}
        config.getConfigSubtreeList("fracture_properties"))
    {
        auto frac_prop = std::make_unique<ProcessLib::LIE::FractureProperty>(
            ...
            //! \ogs_file_param{prj__processes__process__THERMO_HYDRO_MECHANICS_WITH_LIE__fracture_properties__material_id}
            fracture_properties_config.getConfigParameter<int>("material_id"),
            ParameterLib::findParameter<double>(
                //! \ogs_file_param_special{prj__processes__process__THERMO_HYDRO_MECHANICS_WITH_LIE__fracture_properties__initial_aperture}
                fracture_properties_config, "initial_aperture", parameters, 1,
                &mesh),
            ParameterLib::findParameter<double>(
                //! \ogs_file_param_special{prj__processes__process__THERMO_HYDRO_MECHANICS_WITH_LIE__fracture_properties__specific_storage}
                fracture_properties_config, "specific_storage", parameters, 1,
                &mesh),
            ParameterLib::findParameter<double>(
                //! \ogs_file_param_special{prj__processes__process__THERMO_HYDRO_MECHANICS_WITH_LIE__fracture_properties__biot_coefficient}
                fracture_properties_config, "biot_coefficient", parameters, 1,
                &mesh),
            ParameterLib::findParameter<double>(
                //! \ogs_file_param_special{prj__processes__process__THERMO_HYDRO_MECHANICS_WITH_LIE__fracture_properties__initial_fracture_effective_stress}
        ...
        auto permeability_model_config =
            //! \ogs_file_param{prj__processes__process__THERMO_HYDRO_MECHANICS_WITH_LIE__fracture_properties__permeability_model}
    ...
    {
        OGS_FATAL(
            "The number of displacement jumps (%d) and the number of "
            "<fracture_properties> (%d)"
            "are not consistent", n_var_du, fracture_properties.size());
    }

    // initial effective stress in matrix
    auto& initial_effective_stress = ParameterLib::findParameter<double>(
        config,
        //! \ogs_file_param_special{prj__processes__process__THERMO_HYDRO_MECHANICS_WITH_LIE__initial_effective_stress}
    ...
    auto opt_deactivate_matrix_in_flow =
        //! \ogs_file_param{prj__processes__process__THERMO_HYDRO_MECHANICS_WITH_LIE__deactivate_matrix_in_flow}
    ...
    return std::make_unique<ThermoHydroMechanicsProcess<GlobalDim>>(
