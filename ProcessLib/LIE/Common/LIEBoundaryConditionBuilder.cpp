...
#include "LIEBoundaryConditionBuilder.h"
...
#include "CreateLIENeumannBoundaryCondition.h"
...
LIEBoundaryConditionBuilder::createBoundaryCondition(
    ...
    const Process& /*process*/)
{
    ...
    if (type == "Neumann")
    {
        if (bulk_mesh.getDimension()==1) {
            return ProcessLib::LIE::createNeumannBoundaryCondition<1>(
                config.config, config.boundary_mesh, dof_table, variable_id,
        ...
        } else if (bulk_mesh.getDimension()==2) {
            return ProcessLib::LIE::createNeumannBoundaryCondition<2>(
                config.config, config.boundary_mesh, dof_table, variable_id,
                *config.component_id, integration_order, shapefunction_order,
                bulk_mesh.getDimension(), parameters,
                _fracture_props, _junction_props, _frac_ids);
        } else {
            return ProcessLib::LIE::createNeumannBoundaryCondition<3>(
                config.config, config.boundary_mesh, dof_table, variable_id,
                *config.component_id, integration_order, shapefunction_order,
                bulk_mesh.getDimension(), parameters,
                _fracture_props, _junction_props, _frac_ids);
