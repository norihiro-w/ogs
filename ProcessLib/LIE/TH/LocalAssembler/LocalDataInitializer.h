...
private:
    ...
    using LADataMatrix = LocalAssemblerDataMatrix<
        ...
        IntegrationMethod<ShapeFunctionPressure>, GlobalDim>;
    ...
    static LADataBuilder makeLocalAssemblerBuilder()
    {
        return makeLocalAssemblerBuilder<ShapeFunction>(
            static_cast<std::integral_constant<
                bool, (GlobalDim >= ShapeFunction::DIM)>*>(
                nullptr));
    }
        ...
        return [](MeshLib::Element const& e,
            ...
            if (e.getDimension() == GlobalDim)
            {
                return LADataIntfPtr{
                    new LADataMatrix<ShapeFunctionPressure>{
                        e, n_variables, local_matrix_size,
                        dofIndex_to_localIndex,
                        std::forward<ConstructorArgs>(args)...}};
            }
            return LADataIntfPtr{new LAFractureData<ShapeFunctionPressure>{
