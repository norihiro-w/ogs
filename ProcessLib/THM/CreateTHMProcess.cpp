...
std::unique_ptr<Process> createTHMProcess(
    MeshLib::Mesh& mesh,
    ...
    BaseLib::ConfigTree const& config)
{
    ...
    config.checkConfigParameter("type", "THM");
    DBUG("Create THMProcess.");

    auto const staggered_scheme =
        //! \ogs_file_param{prj__processes__process__THM__coupling_scheme}
    ...
    //! \ogs_file_param{prj__processes__process__THM__process_variables}
    ...
    if (use_monolithic_scheme)  // monolithic scheme.
    {
        auto per_process_variables = findProcessVariables(
            variables, pv_config,
            {//! \ogs_file_param_special{prj__processes__process__THM__process_variables__Temperature}
             "temperature",
             //! \ogs_file_param_special{prj__processes__process__THM__process_variables__pressure}
             "pressure",
             //! \ogs_file_param_special{prj__processes__process__THM__process_variables__displacement}
             "displacement"});
    ...
    auto& solid_density = findParameter<double>(
        config,
        //! \ogs_file_param_special{prj__processes__process__THM__solid_density}
    ...
    auto const& solid_linear_thermal_expansion_coefficient = findParameter<
        double>(
        config,
        //! \ogs_file_param_special{prj__processes__process__THM__solid_linear_thermal_expansion_coefficient}
    ...
    auto& solid_specific_heat_capacity = findParameter<double>(
        config,
        //! \ogs_file_param_special{prj__processes__process__THM__solid_specific_heat_capacity}
    ...
    auto& solid_thermal_conductivity = findParameter<double>(
        config,
        //! \ogs_file_param_special{prj__processes__process__THM__solid_thermal_conductivity}
    ...
    auto& intrinsic_permeability = findParameter<double>(
        config,
        //! \ogs_file_param_special{prj__processes__process__THM__intrinsic_permeability}
    ...
    auto& specific_storage = findParameter<double>(
        config,
        //! \ogs_file_param_special{prj__processes__process__THM__specific_storage}
    ...
    auto& biot_coefficient = findParameter<double>(
        config,
        //! \ogs_file_param_special{prj__processes__process__THM__biot_coefficient}
    ...
    auto& porosity = findParameter<double>(
        config,
        //! \ogs_file_param_special{prj__processes__process__THM__porosity}
    ...
    Eigen::Matrix<double, DisplacementDim, 1> specific_body_force;
    {
        std::vector<double> const b =
            //! \ogs_file_param{prj__processes__process__THM__specific_body_force}
    ...
    return std::make_unique<THMProcess<DisplacementDim>>(
...
template std::unique_ptr<Process> createTHMProcess<2>(
    MeshLib::Mesh& mesh,
    std::unique_ptr<ProcessLib::AbstractJacobianAssembler>&& jacobian_assembler,
    std::vector<ProcessVariable> const& variables,
    std::vector<std::unique_ptr<ParameterBase>> const& parameters,
...
template std::unique_ptr<Process> createTHMProcess<3>(
    MeshLib::Mesh& mesh,
    std::unique_ptr<ProcessLib::AbstractJacobianAssembler>&& jacobian_assembler,
    std::vector<ProcessVariable> const& variables,
    std::vector<std::unique_ptr<ParameterBase>> const& parameters,
