...
#ifdef OGS_USE_EIGEN
TEST(NumLib, FemShapeDataWithEigen)
{
    ...
    auto setShapeDataToOnes = [](ShapeDataType &shape)
            {
                shape.N.setOnes();
                shape.dNdr.setOnes();
                shape.dNdx.setOnes();
    ...
    ShapeDataType shape(dim, e_nnodes);
