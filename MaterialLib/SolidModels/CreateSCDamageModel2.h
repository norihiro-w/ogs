...
std::unique_ptr<SCDamageModel2<DisplacementDim>>
createSCDamageModel2(
    std::vector<std::unique_ptr<ProcessLib::ParameterBase>> const& parameters,
    BaseLib::ConfigTree const& config)
{
    //! \ogs_file_param{material__solid__constitutive_relation__type}
    config.checkConfigParameter("type", "SCDamageModel2");
    DBUG("Create SCDamageModel2 material");
    ...
    typename SCDamageModel2<DisplacementDim>::MaterialProperties mp{
        youngs_modulus, youngs_modulus_damaged, poissons_ratio,
        friction_angle, cohesion};

    return std::make_unique<SCDamageModel2<DisplacementDim>>(mp, damage_state);
}
