      ...
      _msh(msh), _problem(problem)
{
    ...
    for (size_t i=0; i<n_var; i++) {
        ...
        MyNodalFunctionScalar* u0 = new MyNodalFunctionScalar(lsLibType);
    ...
    _x_n0 = MathLib::LinAlgBuilder::generateVector(lsLibType, n_total_dofs);
    ...
    _x = MathLib::LinAlgBuilder::generateVector(lsLibType, _dofManager->dofSize());
    _b = MathLib::LinAlgBuilder::generateVector(lsLibType, _dofManager->dofSize());
    _A = MathLib::LinAlgBuilder::generateMatrix(lsLibType, _dofManager->dofSize());
...
//            std::cout << list_bc_values << std::endl;
    ...
    // initial guess
    INFO("->use previous time step value as initial guess");
    *_x_n1_0 = *_x_n0;
